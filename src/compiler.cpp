#include "include/FestaEngine/program/compiler.h"
...
static bool checkMain(const Expression& e) {
	cout << "check: " << e.functional() << endl;
	if (e.name == ";") {
		for (uint i = 0; i < e.size(); i++)
			if (checkMain(e.params[i]))return true;
	}
	else if (e.name != "~v" && e.name != "struct" &&
		e.name != "class" && e.name != "~f") {
		//cout << e.functional() << endl;
		LOGGER.error("This command can only be performed in main(): " + e.serialize());
		return true;
	}
	return false;
}

Command* Compiler::compile(const string& code, LocalMemory*& local, bool initExpression, const CodeErrorLog& logger) {
	Expression expr(code, logger, initExpression);
	if (checkMain(expr))return 0;
	compile(expr, local);

	vector<Command*> commands;
	vector<VAR> vars;
	useFunction("main", commands, vars, local);
	return command;
}
